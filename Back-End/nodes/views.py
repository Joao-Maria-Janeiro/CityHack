...
def create_node(request):
    if request.method == 'POST':
        currentDT = datetime.datetime.now()
        day = currentDT.day
        node = NodeMCU(activation_key=request.POST['activation_key'], curr_day=day, monthly_budget=request.POST['monthly_budget'], power=request.POST['power'], email=request.POST['email'])
        node.save()
...
def update_waste(request):
    if request.method == 'POST':
        node = NodeMCU.objects.get(activation_key=request.POST['activation_key'])
        update_daily(node)
        node.save()
